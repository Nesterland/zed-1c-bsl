...
1. `Expected version: 0.0.3, Actual version: 0.1.2` — в `extensions.toml` форка `Nesterland/extensions` была устаревшая версия 0.0.3, тогда как в `extension.toml` submodule уже 0.1.2
2. `.gitmodules` не отсортирован: запись `1c-bsl` была в конце файла, а должна быть по алфавиту после `1984-theme`

**Исправления:**
1. `extensions.toml` → `version = "0.1.2"` (вместо `0.0.3`)
2. `pnpm run sort-extensions` → `.gitmodules` отсортирован (запись переместилась на позицию после `1984-theme`)

**Результат:** ✅ Все 4 проверки CI пройдены (CLA signed, Danger, Package, Sort). PR #6082 готов к мёрджу.
